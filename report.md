...
Таким образом, в отличие от классической задачи нескольких коммивояжёров, где минимизируется суммарная длина всех маршрутов, в данной задаче требуется минимизировать длину **самого длинного маршрута**, что соответствует задаче балансировки нагрузки между агентами.

Для моего варианта это означает необходимость распределить 12 городов между двумя агентами, стартующими из городов 0 и 6, таким образом, чтобы ни один из маршрутов не оказался существенно длиннее другого.

### 2.2. Математическая модель (QUBO-формулировка / Копипаст из методических материалов с небольшими пометками для себя)
...
Согласно методичке, начальная температура должна подбираться таким образом, чтобы в начале работы алгоритма принималось **около 80% ухудшающих переходов**. Это обеспечивает достаточное исследование пространства решений на ранних этапах поиска.

Для автоматического определения $T_0$ используется следующая процедура:

1. Генерируется случайное начальное решение.
2. Выполняется N случайных шагов генерации соседей.
...
Из этого диапазона выбрано значение (исключительно эксперементальным методом, тестировал разные):
...
Минимальная температура задаёт критерий остановки алгоритма. При $T < T_{\min}$ вероятность принятия любого ухудшающего перехода становится пренебрежимой, и дальнейшая работа SA эквивалентна жадному поиску.
...
- **Максимальное число итераций:** $100000$. Этого значения с большим запасом достаточно для достижения $T_{\min}$ при выбранном $\alpha = 0.97$. Нужно чтоб избежать бесконечного цикла.
...
- makespan у SA меньше;
- суммарная длина у SA меньше;
- баланс у SA прыгает, из-за того что SA - стохастический алгоритм;
...
- SA занимает больше времени, но конкретно под эту задачу, затраты времени помогают найти лучший результат.

## 4. Выводы

В ходе лабораторной работы была изучена задача нескольких коммивояжёров с минимизацией времени обхода (mTSP-makespan), построена её QUBO-формулировка и реализованы два метода решения: жадный алгоритм с локальными улучшениями (Baseline) и квантово-вдохновлённый метод имитации отжига (Simulated Annealing). Оба метода были запущены 10 раз, и их результаты сравнены по ряду метрик.

Основные результаты работы:

1. **Simulated Annealing продемонстрировал лучшее качество решения**, чем Baseline: среднее значение makespan у SA оказалось значительно ниже.

2. **Baseline показал максимальную стабильность и скорость работы**, однако его решение является локально оптимальным — алгоритм не способен выйти из локального минимума и потому уступает SA по основной метрике.

3. **Стохастическая природа SA проявилась в разбросе значений по запускам**: лучшие запуски достигают глобального оптимума, менее удачные останавливаются на промежуточных решениях. Это характерно для методов глобальной оптимизации.

4. **Баланс нагрузки между агентами** у SA в лучших случаях оказывается выше, чем у Baseline, однако в среднем имеет большую вариативность, поскольку SA минимизирует именно makespan, а не баланс.

5. **Подбор параметров SA** оказывает существенное влияние на качество результата: слишком быстрое охлаждение делает SA эквивалентным жадному поиску, а слишком высокая начальная температура замедляет сходимость. Выбранные значения ($T_0$ — автоподбор, $\alpha = 0.97$, $T_{\min} = 0.001$) обеспечивают разумный баланс между качеством и скоростью.

**Общий вывод:** квантово-вдохновлённый метод имитации отжига является более эффективным инструментом решения задачи mTSP-makespan по сравнению с жадным baseline. Несмотря на большее время вычислений, SA позволяет находить решения существенно лучшего качества, что подтверждает целесообразность применения SA для решения подобных задач.
