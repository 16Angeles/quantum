...
1. **Жадный алгоритм с локальными улучшениями (Greedy + 2-opt)** — выступает в роли baseline (эталонного метода для сравнения);
...
Реализация состоит из четырёх последовательных этапов, в точности соответствующих методическим указаниям.

**Этап 1. Сортировка пар городов по расстоянию.**

Все пары городов $(i, j)$ с $i < j$ сортируются по возрастанию расстояния $d_{ij}$. Этот шаг используется как вспомогательная информация и подготавливает алгоритм к жадному назначению.

**Этап 2. Жадное назначение городов агентам.**

На каждом шаге алгоритм:
- выбирает агента с **минимальной текущей длиной маршрута**;
- добавляет к нему **ближайший непосещённый город** относительно последнего города его маршрута.

Процедура повторяется до тех пор, пока все непосещённые города не будут распределены. При равенстве длин маршрутов выбор агента осуществляется случайно.

Такое назначение обеспечивает базовую балансировку: агент с меньшей нагрузкой получает следующий город.

**Этап 3. Применение 2-opt к каждому маршруту.**

Для каждого построенного маршрута выполняется классический локальный поиск **2-opt**:
- перебираются все пары рёбер $(u_{i-1}, u_i)$ и $(u_j, u_{j+1})$ внутри маршрута;
- если разворот сегмента $[u_i, \dots, u_j]$ уменьшает длину цикла, то такой разворот применяется;
- процесс повторяется до тех пор, пока возможны улучшения.

Условие улучшения формально записывается как:

$$
d(u_{i-1}, u_j) + d(u_i, u_{j+1}) < d(u_{i-1}, u_i) + d(u_j, u_{j+1}).
$$

Депо (позиция $0$ маршрута) при 2-opt не затрагивается.

**Этап 4. Перенос городов между агентами для балансировки.**

После локального улучшения отдельных маршрутов выполняется попытка балансировки нагрузки:
- находится маршрут с наибольшей длиной ($L_{\max}$) и маршрут с наименьшей длиной ($L_{\min}$);
- из длинного маршрута выбирается город, перенос которого в короткий маршрут уменьшает значение makespan;
- среди всех возможных переносов выбирается тот, который даёт **наибольшее уменьшение makespan**;
- процедура повторяется до тех пор, пока существуют улучшающие переносы.
...
- максимальное число итераций — $100\,000$.
...
$$
P(\text{accept}) = \exp\!\left(-\frac{\overline{\Delta E}}{T_0}\right) = 0.8,
$$
...
Такой подход даёт значение $T_0$, согласованное с масштабом изменения целевой функции на конкретной задаче. Для варианта №2 типичные значения $T_0$, полученные таким способом, лежат в диапазоне примерно от $5$ до $15$.
...
что соответствует рекомендованным в методичке значениям $0.95$–$0.97$. Этот выбор обеспечивает компромисс между:
- **скоростью работы** — при $\alpha = 0.97$ алгоритм достигает $T_{\min} = 0.001$ примерно за $300$–$350$ итераций при $T_0 \approx 10$;
- **качеством решения** — медленное охлаждение даёт алгоритму достаточно времени для исследования пространства решений.
...
- **Максимальное число итераций:** $100\,000$. Этого значения с большим запасом достаточно для достижения $T_{\min}$ при выбранном $\alpha = 0.97$. Параметр служит дополнительной защитой от зацикливания.
- **Случайный seed:** для каждого из $10$ запусков использован фиксированный seed из диапазона $42$–$51$, что обеспечивает воспроизводимость результатов.
...
| Макс. число итераций  | $100\,000$          | гарантированное достижение $T_{\min}$               |
| Число запусков        | $10$                | требование методички для статистики                 |
| Seed                  | $42$ - $51$         | воспроизводимость                                   |
